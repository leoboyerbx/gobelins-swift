...
struct NewsModelUI {
    ...
    init(title: String, imageUrl: String, hat: String, fullContent: String) {
        self.title = title
...
//    init(record: Record) {
//        if let title = record.fields?.title,
//           let content = record.fields?.freeText {
//            self.title = title
//            self.fullContent = content
//            self.hat = content.substring(to: String.Index(encodedOffset: 50))
//        }
//    }
}
