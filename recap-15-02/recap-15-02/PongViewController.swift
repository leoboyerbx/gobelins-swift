...
class PongViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        Timer.scheduledTimer(withTimeInterval: 1.0/60.0, repeats: true) { (timer) in
            ...
            if self.ball.frame.intersects(self.pad.frame) {
                self.directionY = -5.0
            }
            if self.ball.frame.intersects(topRect.frame) {
                self.directionY = 5.0
            }
            
            if self.ball.frame.intersects(leftRect.frame) {
                self.directionX = 5.0
            }
            
            if self.ball.frame.intersects(rightRect.frame) {
                self.directionX = -5.0
            }
